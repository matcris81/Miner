...
std::string vectorHash(std::vector<unsigned char> data) {
    ...
    const unsigned char * dataArray = &data[0];
    return sha256(dataArray, numBytes);
}

std::string arrayHash(const unsigned char *data) {
    SHA256 sha256;
    int numBytes = *(&data + 1) - data;
    return sha256(data, numBytes);
}
...
std::string uint64Nonce( uint64_t inputnonce ) {
    std::string strnonce = std::to_string(inputnonce);
    int zeroesToAdd = 20 - strnonce.length();
    std::string nonce = "";
    for (int i = 0; i < zeroesToAdd; i++) { nonce.append("0"); }
    nonce.append(strnonce);
    return nonce;
}
...
void string_to_byte_array(std::string hash, BYTE * b) {
    for (int i = 0; i < 32; i++) {
        std::string str;
        str.append(1, hash[(i * 2)]);
        str.append(1, hash[(i * 2) + 1]);
        b[i] = std::stoi(str, 0, 16);
    }
}
