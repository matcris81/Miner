...
extract(const char *filename)
{
    ...
    for (;;)
    {
        ...
        const std::string fullOutputPath = destination + "/smd/" + currentFile;
...
int main(int argc, char *argv[])
{
    ...
    getcwd(tmp, 256);
    std::string destination = convertToString(tmp);
    std::string file_path = destination + "/smd";
    std::uintmax_t n = fs::remove_all(file_path);
    fs::create_directories("smd");
    extract("6eb228fb5b59ab49a45a48bdd9e5f0ed65b43afb52ec72cc825a567986630827.tar");
    int prefix;
    file_path += "/" + returnGreatestFileName(file_path, prefix);

    std::string file = getFileInfo(file_path, prefix, 0);
    std::string newFilePath = file_path.substr(0, file_path.find_last_of('/')) + "/" + file;
    std::string newFileContents = getFileInfo(newFilePath, prefix, 1);
    std::vector<std::string> archiveFiles;
    for (int i = 0; i < newFileContents.length(); i += 64) {
        archiveFiles.push_back(newFileContents.substr(i, 64));
    }
    ...
    {
        std::string finalDestination = destination + "/smd/" + archiveFiles[i];
        std::string xd = getFileInfo(finalDestination, prefix, 1);
        int prefixLength = prefixLengthIdentifier(archiveFiles[i], xd);
    ...
    for (it = hashesOrdered.begin(); it != hashesOrdered.end(); it++) {
        std::string finalFilePath = destination + "/smd/";
